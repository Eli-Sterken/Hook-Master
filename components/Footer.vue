<template>
    <footer class="main-element rel-position normal-element">
        <div class="main-element rel-position accent footer-box">
            <NuxtLink class="main-element rel-position clickible n-font footer-element" to="/">Site Link (Share With Friends!)</NuxtLink>
            <a class="main-element rel-position clickible n-font footer-element" href="https://github.com/Eli-Sterken">My GitHub Page</a>
            <a class="main-element rel-position clickible n-font footer-element" href="https://github.com/Eli-Sterken/Hook-Master">Site GitHub Repo</a>
            <button class="main-element rel-position clickible n-font footer-element" type="button" @click="Feedback">Leave Feedback</button>
            <button class="main-element rel-position clickible n-font footer-element" id="mode-button" type="button" @click="ChangeMode">
                <svg width="100" height="100" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" v-if="mode === 0">
...
<script setup lang="ts">
    ...
    const props = defineProps({
        setMode: {type: Function as PropType<(mode:number) => void>, required: true},
        mode: {type: Number, required: true},
        modal: {type: Function as PropType<ModalAssign>, required: true}
    });

    const hfruifh398y4t9348tyrh34yt3489ty34 = 'MTM0MDgyNTMxNjc2MzEwNzQ4MQ'; // Discord feedback webhook stuff
    const fh3f34tu34tu3490tu4hjrh4389t439th4t = 'WFdMdmFQaUhuaGlYUTRCTVVHNTN4eF80Z0U5WXJENmw0WHo5Rk9hYWxsdmxtbk9PUFpqbVBUX1ZSc2pReWpmRkhiYW8';

    function Feedback() { // Allows user to leave feedback
        props.modal({
            title: "Type your feedback message below and click \"Send\" to send anonymously. Don't worry, we only track your great ideas!",
            input: {
                type: "text",
            ...
            action: async (type, value) => {
                if(type === 'submit') {
                    const { error } = await useFetch(`https://discord.com/api/webhooks/${atob(hfruifh398y4t9348tyrh34yt3489ty34)}/${atob(fh3f34tu34tu3490tu4hjrh4389t439th4t)}`, {
                        method: 'POST',
                        ...
                        body: JSON.stringify({
                            ...
                            "embeds": [
                                {
                                    "title": "New Feedback From Hook Master",
                                    "description": `New feedback from [Hook Master](https://hook-master.pages.dev) has been logged. Feedback message: ${value}`,
                                    "color": 65280,
                                    "author": {
                                        "name": "New Visit",
                    ...
                    if(error.value) { // Alert an error or sucess using the modal
                        setTimeout(props.modal, 0.00000000001, {
                            title: "Oops, there was an error sending your feedback. Please try again later.",
                            submitTitle: 'Close',
                            action: () => {
                                console.error(`Error sending feedback. Status code ${error.value?.statusCode}. `)
                            }
                        });
                    } else {
                        setTimeout(props.modal, 0.00000000001, {
                            title: "Your feedback has been receved 👍",
                            submitTitle: 'Close',
                            action: () => {
                                console.log("Feedback submited sucessfully!")
                            }
                        });
                    };
                };
    ...
    function ChangeMode() { // Changes the mode to the opisite of what it currently is
        if(props.mode === 0) {
            props.setMode(1);
        } else {
            props.setMode(0);
